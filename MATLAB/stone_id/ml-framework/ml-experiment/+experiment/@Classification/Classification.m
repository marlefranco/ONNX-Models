classdef Classification < experiment.Base & experiment.mixin.Classification 
        ...
        function obj = Classification(options)
            ...
            arguments
                ...
                options.Model string {mustBeMember(options.Model,[...
                    "automl" "selectml" "stackml" "tree" "discr" "nb" "knn", ...
                    "svm" "ensemble" "ecoc" "linear" "kernel" "nnet" "treebagger"]) }
    ...
    methods  
        ...
        function cm = confusionchart( obj, options )
            ...
            if ~isempty( obj.validTrialIndices )
                ...
                cm = gobjects(1, numel( options.Item ));
                for iItem = 1 : numel( options.Item )
                    ...
                    accuracy = (nansum(data(tF,:).(actual) == data(tF,:).(prediction)) / numel(data(tF,:).(actual)))*100;
                    
                    modeltype = extractBetween( trial.model(iModel).metadata.modelType,'(',')');
                    titlename = sprintf("trial "+ iTrial + ": " + modeltype + "\n" + ...
                        "Accuracy = " + accuracy + "%%");
                    
                    nexttile()
                    cm(iItem) = viz.confusionchart( data(tF,:), actual, prediction, ...
