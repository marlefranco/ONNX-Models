classdef viz < baseml
    ...
    methods ( Static )
           ...
           %
            ...
            if custom.OmitNaNs == true               
                cm = tril( corrcoef( tbl(:,tF).Variables, ...
                    "Rows", "pairwise"), 0 ); %7/19/21 Switch to pairwise option  
            else
        ...
        function h = boxchart( tbl, x, y, options, custom )
            %BOXCHART Box and whisker chart
            %
            % Syntax:
            ...
            if y ~= ""
                ...
                h = boxchart( custom.ax, grouping, tbl(:,x).Variables, args{:} );
                title( x )
            else
