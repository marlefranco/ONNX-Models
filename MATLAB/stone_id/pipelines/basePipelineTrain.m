...
if options.FeatureExtraction == "descriptivestatistics"
    [value, featurenames] = util.descriptivestatistics(value, ...
...
switch options.FeatureSelection
    ...
    case "fscchi2"
        ...
        [value, featurenames] = util.fscchi2( value, ...
            "PredictorNames", featurenames, ...
...
if isempty(options.TrainIndices)
    value = baseml.partition( value );
else
    value.Partition = repmat("Test", height(value), 1);
    value.Partition(options.TrainIndices) = "Train";
end
